...
func initLogger(cfg *config.Environment) *sloggergo.Logger {
	...
	// fileSink, err := sink.NewFile("logs/app.log", sink.WithFileFormatter(fmt()))
	// if err != nil {
	// 	panic(err)
	// }
...
func main() {
	...
	log.Info("Configuration loaded", slog.Int("port", cfg.Port), slog.String("app", cfg.ServiceName), slog.String("version", cfg.Version), slog.String("environment", cfg.Environment))

	// Connect to PostgreSQL
	log.Info("Connecting to PostgreSQL...")
	db := postgres.MustConnect(&postgres.Config{Url: cfg.PGUrl})
	log.Info("PostgreSQL connected", slog.String("host", cfg.PGHost), slog.String("database", cfg.PGBase))
	...
	log.Info("MinIO connected", slog.String("host", cfg.MinioHost), slog.String("bucket", cfg.MinioBucketName))

	// Connect to Redis
	log.Info("Connecting to Redis...")
	redisSvc := redis.MustNew(redis.Config{Host: cfg.RedisHost, Port: cfg.RedisPort, User: cfg.RedisUser, Password: cfg.RedisPass, DB: cfg.RedisDB})
	log.Info("Redis connected", slog.String("host", cfg.RedisHost), slog.Int("database", cfg.RedisDB))
	...
	log.Info("Application starting", slog.Int("port", cfg.Port), slog.String("go_version", runtime.Version()), slog.Int("num_cpu", runtime.NumCPU()), slog.Int("gomaxprocs", runtime.GOMAXPROCS(0)))
