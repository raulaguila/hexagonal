...
func TestUserRepository_Integration(t *testing.T) {
	...
	err = db.AutoMigrate(&model.UserModel{}, &model.AuthModel{}, &model.RoleModel{})
	require.NoError(t, err)

	// Seed required data (Roles) using Model
	role := &model.RoleModel{Name: "Admin", Permissions: []string{"all"}}
	...
	t.Run("Create and Find User", func(t *testing.T) {
		...
		require.NoError(t, err)
		users, err := entity.NewUser("John Doe", "johndoe", "john@test.com", auth)
		require.NoError(t, err)
