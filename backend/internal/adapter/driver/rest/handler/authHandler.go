...
func NewAuthHandler(router fiber.Router, useCase input.AuthUseCase, accessAuth, refreshAuth fiber.Handler) {
	...
	router.Post("/logout", accessAuth, handler.logout)
...
func (h *AuthHandler) logout(c *fiber.Ctx) error {
	// Parse token manually to get raw token string
	tokenString := c.Get(fiber.HeaderAuthorization)
	if len(tokenString) < 7 || tokenString[:7] != "Bearer " {
		return presenter.Unauthorized(c, "invalid token")
	}
	token := tokenString[7:]

	if err := h.useCase.Logout(c.Context(), token); err != nil {
		return h.handleError(c, err)
	}
